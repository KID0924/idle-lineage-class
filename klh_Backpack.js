/* ============================================================================
 * klh_Backpack.js — 背包功能模組：PC_Fix 列表優化、快速批量賣出/鎖定、renderTabs Hook、廢品記憶清單
 *
 * 設計原則:
 ...
 *   <script src="klh_Backpack.js?v=20260716"></script>
 *
 * 功能一覽:
 ...
 *   4. 廢品記憶清單管理系統 —— 背包中物品點選「設為廢品」將其加入清單，之後獲得該道具時會自動標記。
 *   5. 統計分頁廢品清單按鈕 —— Hook renderAuditTab 注入廢品清單快捷按鈕。
 * ========================================================================== */
...
(function () {
    ...
    function initPowerSavingOptimizations() {
        ...
        if (isChaos) {
            console.log("[klh_Backpack] 偵測為 Chaos 版本，省電重繪與日誌滾動以 Chaos 原生設定為主。");
            return;
        }

        console.log("[klh_Backpack] 非 Chaos 版本，強制開啟 UI 重繪降頻 (每秒2次) 與日誌捲動批過化 (500ms)。");
        ...
        try {
            ...
            console.error("[klh_Backpack] 注入日誌捲動節流失敗:", e);
        }
    }
    ...
    if (typeof window.toggleQuickEnhance === 'function') {
        ...
        window.toggleQuickEnhance = function (type) {
            try {
                ...
                console.error("[klh_Backpack] toggleQuickEnhance hook error:", e);
            }
    ...
    if (typeof window.toggleQuickJunk === 'function') {
        ...
        window.toggleQuickJunk = function (type) {
            try {
                ...
                console.error("[klh_Backpack] toggleQuickJunk hook error:", e);
            }
    ...
    function startupBackpack() {
        if (window.__klhBackpackStarted) return;
        window.__klhBackpackStarted = true;
        injectGlowStyles();
        ...
        if (typeof window.renderAuditTab === 'function' && !window.renderAuditTab.__klhJunkAuditWrapped) {
            ...
            window.renderAuditTab = function () {
                ...
                try {
                    ...
                    console.error("[klh_Backpack] renderAuditTab hook error:", e);
                }
            };
    ...
    // 註冊 DOM 載入與即時啟動
    function safeStartupBackpack() {
        try {
            startupBackpack();
        } catch (e) {
            console.error("[klh_Backpack] startup error:", e);
        }
        try {
            initPCFix();
        } catch (e) {
            console.error("[klh_Backpack] initPCFix error:", e);
        }
    }

    document.addEventListener('DOMContentLoaded', safeStartupBackpack);
    ...
    window.addEventListener('load', function () {
        try {
            ...
            console.error("[klh_Backpack] window load error:", e);
        }
    });
