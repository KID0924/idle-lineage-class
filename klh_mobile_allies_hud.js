 ...
 *
 * 功能一覽:
 *   1. 自動顯示隱藏     —— 僅在手機模式 (body.m-mobile) 且處於戰鬥檢視時自動啟動顯示。
...
(function () {
    ...
    function updateHudUI() {
        try {
            ...
            // 條件過濾：必須在手機模式 (m-mobile)、戰鬥面板 (mview-battle)，且至少有一隻已招募傭兵 (最多取前三個)
            const isMobile = document.body.classList.contains('m-mobile');
            const isBattle = document.body.classList.contains('mview-battle');
            ...
            if (!isMobile || !isBattle || allies.length === 0) {
